...
class Tuning:
    ...
    def write(self, name, value):
        """
        Writes a byte array to a device over USB using a vendor-specific control
        transfer.

        Args:
            name (int): 4-byte device class identifier of the USB device being controlled.
            value (str): 4-byte value that will be written to the device at the
                specified offset.
    ...
    def read(self, name):
        """
        Is a control transfer command that reads data from a USB device. It takes
        in a device ID, command code, and length of data to read and returns the
        read data as an integer value.

        Args:
            name (list): 4-byte value of an USB control request code.
    ...
    def version(self):
        """
        Sends a control request to the connected device with vendor and recipient
        ID set to `USB.DEVICE.CONTROL.RECIPIENT.DEVICE` and data values set to
        `0x80`. It returns the result of the request, which is stored in the `[0]`
        index of the returned value tuple.

        Returns:
            int: a single-valued integer representing the USB device's version number.
...
def find(vid=0x2886, pid=0x0018):
    """
    Finds an active USB device with the given vendor and product IDs. If the device
    is found, it detaches the kernel driver associated with a specific interface
    number on the device.

    Args:
        vid (`usb.core.UsbVendorId`.): 16-bit vendor ID of the USB device to be
            searched for in the core's device database.
            
            		- `idVendor`: The vendor ID of the device, which can range from 0
            to 65534.
            		- `idProduct`: The product ID of the device, which can range from 0
            to 65534.
        pid (int): 16-bit product code identifier for the USB device to be detected
            and configured.
...
def main():
    """
    Takes care of documenting parameters of given code for output into a file.
