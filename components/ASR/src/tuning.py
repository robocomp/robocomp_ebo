...
class Tuning:
    ...
    def write(self, name, value):
        """
        Writes a USB value to a device using the vendor-specific control transfer
        protocol. It retrieves the desired parameter from a dictionary and packs
        the payload data into a specific format, depending on the parameter type.
        Then it sends the payload to the device via the control transfer request.

        Args:
            name (str): 32-bit device class code of the target device for which
                the function is executing.
            value (str): 4-byte value to be written to the USB device's endpoint.
    ...
    def read(self, name):
        """
        Controls a USB device using a control transfer command and returns a value
        based on the command and device ID.

        Args:
            name (str): 8-bit value of the command code for the USB device control
                request.
    ...
    def version(self):
        """
        Controls transfer of vendor-specific information from the host to the
        device using CTRL_IN command with a type code of 0x80, acknowledging that
        data is sent successfully with value of 0, and then sending a response of
        1 indicating successful transfer.

        Returns:
            int: a single-byte integer representing the device's USB version number.
...
def find(vid=0x2886, pid=0x0018):
    """
    Is used to search for an USB device based on its vendor and product ID, and
    returns it if found. If the device is found, it retrieves the active configuration
    and interface number of the device, and then detaches the kernel driver
    associated with the interface number.

    Args:
        vid (int): 16-bit Vendor ID of the USB device, which is used to find the
            appropriate device instance in the `usb.core` module.
        pid (int): 16-bit product code ID of the USB device being searched, which
            is used to identify and find the device in the system.
...
def main():
    """
    Is given a list of command-line arguments, which it processes accordingly. If
    there are more than one argument, it prints information about the available
    device and its parameters. Otherwise, it finds the nearest device and reads
    its version and some of its parameters, depending on the value of the first
    argument (which must be either `-p`, `-r`, or a valid device name).
