...
class GHistoryState():
    ...
    def __init__(self, states: List[Tuple[ChangeInfo, Union[None, Node, Edge, Dict[int, Node]]]]):
        ...
        """
        Sets up a `states` list, a `node_states` dictionary, and an `edge_states`
        dictionary to store GHistoryState objects. It also adds default entries
        for empty nodes, edges, or state changes.

        Args:
            states (List[Tuple[ChangeInfo, Union[None, Node, Edge, Dict[int,
                Node]]]]): 2D grid of history states, where each cell (positioned
                at both `ci.node_or_from_id` and `ci.node_or_to_id`) contains an
                object with `ci.op` indicating the type of change occurred at that
                position, and other attributes providing additional information
                about the change.
    ...
    def _lower_bound(self, state : int, arr : List[Union[Tuple[int, Node, int, int], Tuple[int, Edge, int, int]]]) -> Union[int, None]:
        """
        Finds the lower bound of a given number in an array of integers, based on
        the value's position in the array.

        Args:
            state (int): value used to compare with the elements of the given array
                to determine whether it is found, and if so, the index at which
                it was found.
            arr (List[Union[Tuple[int, Node, int, int], Tuple[int, Edge, int,
                int]]]): 2D array whose elements are being searched for a given
                target value `state`.

        Returns:
            Union[int, None]: the index of the leftmost occurrence of a given
            number in an array.
    ...
    def get_change_state(self, state: int) -> Tuple[Union[Node, Edge, None], Union[Node, Edge, None]]:
        ...
        """
        Evaluates given ci object and returns a tuple containing the new state
        value or None if no change occurs and an object representing the change
        node or edge.

        Args:
            state (int): current state of the graph and determines which parts of
                the code to execute depending on its value.

        Returns:
            Tuple[Union[Node, Edge, None], Union[Node, Edge, None]]: a tuple of
            two values: a none value and an object representing either a node or
